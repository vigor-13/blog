...
<!-- Vercel에서 배포를 생성하는 가장 일반적인 방법은 Git 저장소에 코드를 푸시하는 것입니다.

Vercel에서 Git 저장소를 가져오면 자동 배포 프로세스가 시작됩니다. 이 저장소는 비공개 또는 공개일 수 있습니다. Git 저장소를 설정하려면 Vercel 대시보드에서 New Project 버튼을 클릭하고 다음 Git 공급업체 중 하나를 선택하세요.

GitHub
GitLab
Bitbucket

Vercel의 Git 통합을 사용하여 애플리케이션을 배포하면 다음과 같은 이점이 있습니다.

풀 요청 또는 병합 요청을 통한 간소화된 협업
주요 브랜치에 병합하여 프로덕션 배포를 자동으로 생성하기 전에 프리뷰 배포를 통해 새로운 기능과 변경 사항을 테스트할 수 있습니다(지원되는 Git 공급업체에서).
Comments 기능을 사용하여 프리뷰 환경에서 라이브 피드백을 받을 수 있습니다.

또한 Vercel 대시보드에서 직접 Git 참조로 배포를 생성할 수 있습니다. 자동 배포에 중단이 있을 때 특정 커밋이나 브랜치의 최신 변경 사항을 배포해야 하는 경우에 유용합니다.

Git 저장소 배포에 대한 자세한 내용은 가이드를 참고하세요.

Git 저장소의 외부 이벤트를 기반으로 배포를 시작하려면 Deploy Hooks를 사용할 수 있습니다. -->
...
<!-- Vercel REST API를 사용하면 관련 엔드포인트로 HTTP POST 요청을 보내 배포를 생성할 수 있습니다. 요청 본문에는 배포할 모든 파일이 포함되어 있습니다. 각 파일에 대해 SHA를 생성하고 Vercel REST API를 사용하여 업로드해야 합니다.

이 배포 방법은 사용자 지정 워크플로우를 사용하는 프로젝트, 멀티 테넌트 애플리케이션에 사용자 지정 도메인을 추가하거나 Vercel 플랫폼에서 현재 지원하지 않는 서드파티 서비스를 통합하는 경우에 도움이 됩니다.

Vercel REST API 엔드포인트에 대한 자세한 내용은 API 참조를 참조하세요. -->
...
- Edge Middleware
- 정적 자산(예: HTML, CSS, JavaScript 파일)
- 함수(서버리스 함수 또는 Edge 함수)
- ISR 함수

배포 요약은 유형별로 구성된 모든 이러한 출력물에 대한 자세한 개요를 제공합니다. 이 요약을 통해 빌드 출력을 이해하고 배포되는 내용을 확인할 수 있습니다.

배포 요약에 액세스하려면 프로젝트의 Deployment 탭에서 배포를 선택합니다. Deployment Status에서 Deployment Summary 탭을 확장합니다. -->
