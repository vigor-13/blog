---
title: Expo로 앱 개발하기
description: 핵심 개발 루프에 대한 정신적 모델을 구축하는 데 도움이 되는 엑스포 앱 개발 프로세스의 개요다.
date: 2024-04-02
tags: []
references:
...
### Expo app이란 무엇인가? {#what-is-an-expo-app}

Expo app은 _Expo 도구를 사용하는 React Native 앱_ 을 나타내는 약어다. "Expo app"은 Expo SDK 단일 패키지 또는 Expo Router, Expo CLI, Continuous Native Generation, 이들의 조합 또는 기타 다른 Expo 도구를 사용할 수 있다.

:::note "Expo app" 과 "Expo 도구를 사용하지 않는 React Native app"의 개발 프로세스가 다를까?
...
### Expo와 Expo Application Service(EAS)의 차이점은? {#what-is-the-difference-between-expo-and-eas}
...
`npx create-expo-app` 으로 새 앱을 초기화할 때, **android** 또는 **ios** 디렉토리가 보이지 않을 것이다. [`npx expo prebuild` 를 실행하여 네이티브 프로젝트를 생성](https://docs.expo.dev/workflow/prebuild/)할 수 있으며, 이 명령은 네이티브 프로젝트를 초기화한 다음 프로젝트의 Expo 앱 구성(**app.json/app.config.js**)을 적용한다.
...
:::note Expo 프로젝트를 create-expo-app으로 초기화할 때 기본적으로 네이티브 프로젝트가 생성되지 않는 이유
...
### Continuous Native Generation (CNG) {#continuous-native-generation}

Continuous Native Generation (CNG)은 **app.json**과 **package.json**을 사용하여 필요할 때마다 [네이티브 프로젝트](https://docs.expo.dev/workflow/overview/#android-and-ios-native-projects)를 생성하는 Expo 앱 빌드 프로세스다. 이는 **package.json**에서 **node_modules**가 생성되는 방식과 유사하다.

[네이티브 프로젝트](https://docs.expo.dev/workflow/overview/#android-and-ios-native-projects) 디렉토리(**android**와 **ios**)를 **.gitignore**에 추가하거나 프로젝트를 언제든지 삭제한 다음, 필요할 때마다 `npx expo prebuild` 를 사용하여 Expo 앱 구성을 통해서 다시 생성할 수 있다. 클라우드 기반 개발 워크플로우를 사용하는 경우 개발 컴퓨터에서 직접 prebuild를 실행하지 않을 수도 있다.

CNG를 사용하면 React Native의 새 버전으로 업그레이드하는 것이 훨씬 쉬워진다. 프로젝트 유지 관리를 단순화하고 [App Clips](https://github.com/bndkt/react-native-app-clip), [공유 익스텐션](https://github.com/timedtext/expo-config-plugin-ios-share-extension), [푸시 알림](https://github.com/OneSignal/onesignal-expo-plugin)과 같은 복잡한 기능의 설정을 용이하게 한다. 이는 모두 [구성 플러그인](https://docs.expo.dev/config-plugins/introduction/)을 통해 가능하다.

:::note 만약 prebuild로 프로젝트를 생성하는 대신 Android Studio나 Xcode에서 직접 네이티브 프로젝트 구성을 편집하고 싶다면 어떻게 해야 할까?
...
### 클라우드 기반과 로컬 개발 워크플로 {#cloud-base-and-local-development-workflows}
...
2. 로컬 개발:
   ...
   - 네이티브 프로젝트 디렉토리(**android** 및 **ios**)를 생성하기 위해 `expo prebuild` 를 실행한다.
...
- **JavaScript 코드 작성 및 실행**: 이는 컴포넌트 생성, 비즈니스 로직 작성 또는 네이티브 코드 변경이 필요하지 않은 npm 라이브러리 설치를 포함한다. 여기서 변경한 사항은 앱의 네이티브 측면과의 상호 작용 없이 앱에 반영된다.

- **앱 구성 업데이트**: 이는 앱 구성 파일(**app.json** 또는 **app.config.js**)을 사용하여 앱의 구성을 수정하는 것을 포함한다. 앱의 이름, 아이콘, 스플래시 화면 및 기타 속성 업데이트가 포함된다. 이러한 변경 사항이 모두 네이티브 프로젝트에 직접 영향을 미치는 것은 아니다. 그러나 네이티브 프로젝트에 영향을 미치는 변경을 하는 경우 [앱 구성](https://docs.expo.dev/workflow/configuration/)을 사용하여 네이티브 프로젝트 구성을 수정하거나 [구성 플러그인](https://docs.expo.dev/config-plugins/introduction/)을 생성하거나 사용할 수 있다. 앱 구성 파일에서 사용 가능한 속성의 전체 목록은 [앱 구성 레퍼런스](https://docs.expo.dev/versions/latest/config/app/)를 참조한다.

- **네이티브 코드 작성 또는 네이티브 프로젝트 구성 수정**: 이는 네이티브 코드를 직접 작성하거나 네이티브 코드 구성을 수정하는 것을 포함한다. 이러한 변경을 수행하려면 네이티브 코드 프로젝트 디렉토리에 대한 액세스 권한이 필요하거나 [로컬 Expo Module](https://docs.expo.dev/modules/get-started/#adding-a-new-module-to-an-existing-application)을 사용하여 네이티브 코드를 작성할 수 있다.

- **네이티브 코드 수정이 필요한 라이브러리 설치**: 이는 라이브러리가 네이티브 코드 프로젝트 구성을 변경해야 함을 의미한다. 라이브러리는 구성 플러그인이나 앱 구성을 업데이트하기 위한 단계를 제공한다. 이전 활동과 마찬가지로 이 작업에도 개발 빌드를 생성해야 한다.
...
:::note
개발 빌드를 로컬에서 생성할 때 `npx expo run` 명령은 앱을 빌드하기 전에 네이티브 디렉토리를 생성한다. 첫 번째 빌드 후에 프로젝트 구성이나 네이티브 코드를 수정하는 경우 프로젝트를 다시 빌드해야 한다. `npx expo prebuild` 를 다시 실행하면 변경 사항이 기존 파일 위에 계층화된다. 또한 빌드 후에 다른 결과를 생성할 수 있다. 이를 방지하려면 프로젝트의 **.gitignore**에 네이티브 디렉토리를 추가하고 `npx expo prebuild --clean` 명령을 사용한다.
:::
