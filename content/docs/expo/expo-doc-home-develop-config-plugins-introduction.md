...
프로젝트에 네이티브 모듈을 추가하기 위한 자동 설정이 가능하다. 하지만 때로는 모듈이 더 복잡한 설정을 요구하기도 한다. 이 때 설정 플러그인(config plugin)을 사용하면 모듈에 맞게 네이티브 프로젝트를 자동으로 구성하고, 네이티브 프로젝트와의 직접적인 상호 작용을 피함으로써 복잡성을 줄일 수 있다.
